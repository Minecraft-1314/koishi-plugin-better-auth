...
class LoginHandler {
    constructor(ctx, config, tokenManager, notification, debug, extensions, setAuth) {
        this.ctx = ctx;
        this.config = config;
    ...
    async recordAttempt(username, address, fingerprint, success) {
    ...
    async handlePasswordLogin(client, name, password, remember, deviceFingerprint) {
        ...
        if (user.password && !user.password.includes(':')) {
            await this.ctx.database.set('user', { id: user.id }, { password: (0, password_1.hashPassword)(password) });
        }
    ...
    async handleTokenLogin(client, aid, token) {
    ...
    async handleRefreshLogin(client, refreshToken) {
